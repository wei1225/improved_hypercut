...
class dbintree {
  ...
  void add_stub(struct tnode *node, InetAddress128& addr, dlist* Flist, struct filterV6 filters[],int CurrNest);
  void add2child_stublist(struct tnode *node, int dir, int filt);
  void add_node(struct tnode *prnt, int lev, int dir);
  void finish_node(struct tnode *node, InetAddress128& addr, dlist* Flist, struct filterV6 filters[],int CurrNest);

...
PrefixList::PrefixList() {
  ...
  for (int type = 0; type < 25; type++){
    ...
    for (int i = 0; i < N; i++) {
      ...
      for (int j = 0; j < ADDRLEN + 1; j++) prefixes[type][i].sprob[j] = 0;//modified j < 33 to j < 129
...
void PrefixList::smooth_type(int type, int s){
  ...
  //double sp[129];
  //double spj[129];
  //struct prefix temps[65];//65 is the total length of sa and da?88888888888888
  double tp[4*ADDRLEN+1];
  double sp[4*ADDRLEN+1];
  double spj[4*ADDRLEN+1];
  struct prefix temps[2*ADDRLEN+1];
  ...
  tk = 2*s; //double smothness?88888888888888888
  binomial(tk,tp);
  ...
  for (int i = 0; i < N; i++) {
	...
	tlen = i - s; bdist = 0;
      ...
      while (tlen <= 2*ADDRLEN && bdist <= tk) {

	if (tlen < 0 || tlen > 2*ADDRLEN){
	...
	if (prefixes[type][i].sprob[j] > 0){
	  ...
	  while (slen <= ADDRLEN && bdist <= skj) {
	    
	    if (i < 0 || i > 2*ADDRLEN || slen < 0 || slen > ADDRLEN){
	  ...
	  for (int m = 1; m <= s; m++) {
		...
		bdist = 0;
	      ...
	      while (start <= ADDRLEN && bdist <= sk) {
		
		if (r < 0 || r > 2*ADDRLEN || start < 0 || start > ADDRLEN){
  ...
  if (totw > 0) {
	...
	if (i < ADDRLEN) {
	  for (int j = i + 1; j <= ADDRLEN; j++) temps[i].sprob[j] = 0;
	}
	else if (i > ADDRLEN) {
	  for (int j = 0; j < i - ADDRLEN; j++) temps[i].sprob[j] = 0; 
	}
	// Normalize source distribution
	tots = 0;
	for (int j = 0; j <= ADDRLEN; j++) tots += temps[i].sprob[j]; 
	for (int j = 0; j <= ADDRLEN; j++) temps[i].sprob[j] = temps[i].sprob[j] / tots; 
      }
    }
	...
	if (j == ADDRLEN) prefixes[type][i].sprob[j] = 1;
      }
    }
  }
  // print(stdout);
...
struct ppair PrefixList::choose_prefix(int type, float rs, float rt) {
  ...
  for (int i = 0; (i < N && done == 0); i++) {
    ...
    if (rt <= prefixes[type][i].prob) {
      for (int j = 0; (j < ADDRLEN+1 && done == 0); j++) {

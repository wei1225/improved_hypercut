...
void sbintree::build_tree(dlist* Flist, struct filterV6 filters[]){
  //unsigned int addr = 0;
  InetAddress128 addr;
  // Create copy of list
...
void sbintree::add_node(struct stnode *prnt, int lev, int dir, InetAddress128& addr, dlist* Flist, struct filterV6 filters[], int CurrNest){
  // prnt: 当前树节点(父节点）
  // lev:  所要加的节点所在的层?
  // dir:  所要加的节点的方向. 0:左边,1:右边
  // addr: 地址,
  int Flist_size = 0;
  /*
  printf("add_node:\n");
  ...
  if (Flist_size == 0) return;
    ...
    if (lev == 0) {
     ...
     // addr1 = 1;
      ...
      addr0.address[0] = 0;
      addr1.address[0] = 1;
      addr1.address[0] = addr1.address[0] << 31;
    } 
    else {
      //addr0 = addr >> (32 - lev);
      //addr0 = addr0 << (32 - lev);

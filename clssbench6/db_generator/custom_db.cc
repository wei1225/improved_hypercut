...
int custom_db_gen(int num_filters, FilterList* filters, FILE* fp_in, int smoothness, float addr_scope, float port_scope, int branch){
  ...
  // Temporary filter
  // struct filterV6 temp_filter;
  //struct filterV6 *temp_filters = new struct filterV6[num_filters+1];
  struct filterV6 *temp_filters = new struct filterV6[num_filters+1];
  ...
  for (int i = 1; i <= num_filters; i++){
    ...
    temp_filters[i].da_len = temp_ppair.dlen;
		temp_filters[i].sa.SetPrefixLen(temp_ppair.slen);
    temp_filters[i].da.SetPrefixLen(temp_ppair.dlen);
  ...
  */
  ...
  delete(Flist);
	...
	SetLast64bit(num_filters, temp_filters);
	/*
	int tmpiii = 0;
	for(tmpiii = 1 ; tmpiii <num_filters ; ++tmpiii)
	{
			(*filters)&=temp_filters[tmpiii];
	}*/
...
// Port selection
void select_ports(int port_type, struct filterV6 *temp_filter, PortList *sparL, PortList *spemL, PortList *dparL, PortList *dpemL){
  double p;
...
int SetLast64bit(int num_filters,struct filterV6 filters[])
{
    ...
    for (int i = 1; i <= num_filters; i++){
        ...
        if (tmp <0.5)
        {
            productorID = randint(0,0x00E6D3);
        }    
        else
        {
            productorID = randint(0x080001,0x08BBCC);
         }
        filters[i].sa.address[2] = productorID << 24;
        filters[i].sa.address[2] = filters[i].sa.address[2] |0xff;
        filters[i].sa.address[3] = (0xfe <<24) |randint(0,0xFFFFFF);
        tmp = drand48();
        if (tmp <0.5)
        {
            productorID = randint(0,0x00E6D3);
        ...
        filters[i].da.address[2] = productorID << 24;
        filters[i].da.address[2] = filters[i].da.address[2] |0xff;
        filters[i].da.address[3] = (0xfe <<24) |randint(0,0xFFFFFF);
    }

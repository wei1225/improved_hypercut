...
int bucketSize=32,NClmt1=256; // leaf threashold
float spfac1 = 2.0,spfac2=4.0,spfac3=600.0,a1=0.6,b1=0.3,c1=0.1;   // space explosion factor
int redun = 0;
int push = 0;
int pushthresh = 4;
...
int loadrule(FILE *fp, struct pc_rule *rule){
  ...
  unsigned proto, protomask,flag,flagmask;
  ...
  while(1){
    ...
    if(fscanf(fp,"@%d.%d.%d.%d/%d\t%d.%d.%d.%d/%d\t%d : %d\t%d : %d\t%x/%x\t%x/%x\n", 
        ...
        &proto, &protomask,&flag,&flagmask) != 18) break;
    if(siplen == 0){
...
void parseargs(int argc, char *argv[]) {
  int	c;
  bool	ok = 1;
  while ((c = getopt(argc, argv, "b:s:x:y:z:o:p:r:t:hdq:")) != -1) {
    switch (c) {
	case 'b':
	...
	case 'q':
	  ...
	  break;
	case 'h':
	  printf("hypercut [-b bucketSize][-s spfac1][-d][-p pushthresh][-r ruleset][-t trace]\n");
	  exit(1);
	  break;
	default:
	  ok = 0;
...
int main(int argc, char* argv[]){
  ...
  trie T(500*numrules, numrules, bucketSize,NClmt1,spfac1,spfac2,spfac3,a1,b1,c1,rule, redun, push, pushthresh);
  ...
  if(fpt != NULL){
    i=0; j=0;
    while(fscanf(fpt,"%u %u %d %d %d %d	%d\n", 
